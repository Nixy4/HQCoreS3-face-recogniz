...
static lv_obj_t* lv_create_page_recog()
{
  ...
  lv_obj_set_style_pad_right(page, 10, 0);
	...
	lv_img_set_src(lvo_cam, &lvid_cam);//图像源
  lv_obj_set_size(lvo_cam, lv_pct(80), lv_pct(70));//宽度,高度
  ...
  lv_label_set_text(label_btn, "设置");
...
static void bep_recog()
{
  ...
  memcpy((void*)lvid_cam.data, frame->buf, lvid_cam.data_size);
  if(osque_frame_out){
...
void fr_main_init(QueueHandle_t frame_out, QueueHandle_t http_req)
{
  ...
  //*LVGL缓冲区和页面初始化
  lvid_cam.data = (uint8_t *)heap_caps_malloc(lvid_cam.data_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  assert(lvid_cam.data!=NULL);
	//Style初始化
	lv_style_init(&lvs_default);
	lv_style_set_radius(&lvs_default, 5);
  //设置字体
  lv_style_set_text_font(&lvs_default, &msyh14);
