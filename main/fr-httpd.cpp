...
static const char* TAG = "FR-HTTPD";

static SemaphoreHandle_t osmtx_frame = NULL;
static QueueHandle_t osque_frame_in  = NULL;
...
static size_t jpg_encode_stream(void *arg, size_t index, const void *user, size_t len)
{
	...
	if (httpd_resp_send_chunk(j->req, (const char *)user, len) != ESP_OK){
		return 0;
	}
	j->len += len;
	return len;
...
static esp_err_t capture_handler(httpd_req_t *req)
{
	...
	if (xQueueReceive(osque_frame_in, &frame, portMAX_DELAY))
	{
		...
		// if (osque_frame_out)
		// {
		// 	xQueueSend(osque_frame_out, &frame, portMAX_DELAY);
		// }
		// else if (gReturnFB)
		// {
...
static esp_err_t stream_handler(httpd_req_t *req)
{
	...
	if(osque_frame_in)
	{
		...
		//获取frame锁
		if(xSemaphoreTake(osmtx_frame, portMAX_DELAY)==pdTRUE)
		{
			//ESP_LOGE("httpd", "get frame lock\n");
		}
		else
		{
			ESP_LOGE("httpd", "get frame lock failed\n");
...
static esp_err_t status_handler(httpd_req_t *req)
{
	...
	*p++ = '{';
	...
	p += sprintf(p, "\"board\":\"%s\",", BSP_CAM_MODULE_NAME);
...
{
	size_t json_len = 0;
	const char *json_response = fr_mdns_query(&json_len);
	httpd_resp_set_type(req, "application/json");
	httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
extern fr_user_t user_list[FR_USER_MAX];
...
static esp_err_t register_handler(httpd_req_t *req)
{
    ...
    content[recv_size] = '\0';
    ESP_LOGI(TAG, "Received user: %s", content);
    ...
    const cJSON *employee_id = cJSON_GetObjectItemCaseSensitive(json, "employeeId");
    const cJSON *position = cJSON_GetObjectItemCaseSensitive(json, "position");

    if (
			cJSON_IsString(username) && 
			cJSON_IsNumber(age) && 
			cJSON_IsString(gender) && 
			cJSON_IsString(employee_id) && 
			cJSON_IsString(position)
			) 
		{
			//把数据存储到user_data中
			strcpy(user.name, username->valuestring);
			user.age = age->valueint;
			strcpy(user.gender, gender->valuestring);
			strcpy(user.employee_id, employee_id->valuestring);
			strcpy(user.position, position->valuestring);

			//打印数据
			ESP_LOGI(TAG, "接收到数据:[name:%s][age:%d][gender:%s][employee_id:%s][position:%s]", 
				user.name, user.age, user.gender, user.employee_id, user.position);

			//把数据发送给fr-main,覆盖原有数据
			if(xQueueSend(osque_http_req, &user, portMAX_DELAY)==pdTRUE) {
				ESP_LOGI(TAG, "Send user data to fr-main success");
			} else {
				ESP_LOGE(TAG, "Send user data to fr-main failed");
			}
    }
    cJSON_Delete(json);
...
void ostk_fr_httpd(void* arg)
{
	...
	{
		//等待frame锁
		if (xSemaphoreTake(osmtx_frame, 0) == pdTRUE)
		{
			camera_fb_t *frame = NULL;
			if(xQueueReceive(osque_frame_in, &frame, portMAX_DELAY)==pdTRUE)
			{
				//释放frame
				// if (osque_frame_out) {
				// 	xQueueSend(osque_frame_out, &frame, portMAX_DELAY);
...
{
	//初始化osmtx_frame
	osmtx_frame = xSemaphoreCreateMutex();
	osque_http_req = http_req;

	osque_frame_in = frame_i;
	// osque_frame_out = frame_o;
	// gReturnFB = return_fb;
